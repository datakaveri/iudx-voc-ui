...
import { environment, environmentAgri } from '../environments/environment';
...
export class InterceptorService {
  ...
  constructor(private http: HttpClient, private router: Router) {
    this.api_url = (
      localStorage.getItem('theme') === 'adex' ? environmentAgri : environment
    ).BASE_URL;
  ...
  get_api_headersLD(route) {
    return new Promise((resolve, reject) => {
      this.http
        .get(this.api_url + route, { headers: this.headersLD })
        .subscribe(
          (data: any) => {
            resolve(data);
          },
